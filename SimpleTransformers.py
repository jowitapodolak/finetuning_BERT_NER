...
def main(trainingdataset, testdataset, outputdir):
    ...
    #should it be test st or eval set?
    # Evaluate the model
    #  def eval_model(self, eval_data, output_dir=None, verbose=True):
    #result, model_outputs, predictions= model.eval_model(testdataset) #why did you call it wrong predictions?
    # eval_file: eval_data should  be the path to a.txt file containing the evaluation data or a pandas DataFrame.
    
    #eval_init_df.to_csv('/Users/philli/Keras_mBERT_TestServer/SimpleTransformers_Run/outputTest_Config', index=False) #<- what is it for?

    #print(result)
    # # # PRINT IT TO A FILE - 03.04.2020 - must be str to write into file (not numpy) error
    # file0 = open("results.txt", "w")  # 'a' would append to an existing file
    # file0.write(model_outputs)
    # file0.close()
    # file1 = open("model_outputs.txt", "w") #'a' would append to an existing file
    # file1.write(model_outputs)
    # file1.close()
    #print(model_outputs)
    # file2 = open("predictions.txt", "w")  # 'a' would append to an existing file
    ...
    # #prediction
    # #again word predicitons - change the name?
    # # Predictions on arbitary text strings (testing sentence annotated)
    # predictions, raw_outputs = model.predict(["Some arbitary sentence with New York and Trump + WWF"])
    # file3 = open("sentences.txt", "w") #'a' would append to an existing file
    # file3.write(predictions)
    # file3.close()
    # # #print(model_outputs)
    # #print(predictions)
...
#NER Model adapted and changed from simpletransformers
